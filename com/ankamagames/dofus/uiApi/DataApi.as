package com.ankamagames.dofus.uiApi
{
   ...
   public class DataApi extends Object implements IApi
   {
      ...
      public function getSetEffects(param1:Array, param2:Array = null) : Array
      {
         ...
         var _loc3_:Array = this.deepClone(param2);
         var _loc4_:Dictionary = new Dictionary();
         var _loc5_:Array = new Array();
         var _loc6_:Array = new Array();
         var _loc7_:Array = new Array();
         for each(_loc8_ in PlayedCharacterManager.getInstance().inventory)
         {
            if(_loc8_.position <= 15)
            {
               for(_loc13_ in param1)
               {
                  if(_loc8_.objectGID == param1[_loc13_])
                  {
                     _loc7_.push(_loc8_);
                     param1[_loc13_] = -1;
                  }
         ...
         for each(_loc9_ in param1)
         {
            if(_loc9_ != -1)
            {
               for each(_loc14_ in Item.getItemById(_loc9_).possibleEffects)
               {
                  if(Effect.getEffectById(_loc14_.effectId).useDice)
                  {
                     if(_loc4_[_loc14_.effectId])
                     {
                        _loc4_[_loc14_.effectId].add(_loc14_);
                     }
                     else
                     {
                        _loc4_[_loc14_.effectId] = _loc14_.clone();
                     }
                  }
                  else
                  {
                     _loc6_.push(_loc14_.clone());
                  }
               }
            }
         }
         ...
         {
            for each(_loc15_ in _loc10_.effects)
            {
               if(Effect.getEffectById(_loc15_.effectId).useDice)
               {
                  if(_loc4_[_loc15_.effectId])
                  {
                     _loc4_[_loc15_.effectId].add(_loc15_);
                  }
                  else
                  {
                     _loc4_[_loc15_.effectId] = _loc15_.clone();
                  }
               }
               else
               {
                  _loc6_.push(_loc15_.clone());
               }
            }
         }
         if((_loc3_) && (_loc3_.length))
         {
            for each(_loc16_ in _loc3_)
            {
               ...
               {
                  if(_loc4_[_loc16_.effectId])
                  {
                     _loc4_[_loc16_.effectId].add(SecureCenter.unsecure(_loc16_));
                  }
                  else
                  {
                     _loc4_[_loc16_.effectId] = SecureCenter.unsecure(_loc16_).clone();
                  }
               }
               else
               {
                  _loc6_.push(this.deepClone(SecureCenter.unsecure(_loc16_)));
               }
         ...
         {
            if(_loc12_.showInSet > 0)
            {
               _loc5_.push(_loc12_);
            }
         }
         _loc5_.sortOn("category",Array.NUMERIC);
         return _loc5_;
      }
