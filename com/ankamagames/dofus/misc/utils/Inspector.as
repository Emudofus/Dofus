package com.ankamagames.dofus.misc.utils
{
    ...
    public class Inspector 
    {
        ...
        private function changeBeriliaInteraction():void
        {
            ...
            {
                for (_local_1 in this._berilaChangedInteraction)
                {
                    if ((((((_local_1 is InteractiveObject)) && ((this._berilaChangedInteraction[_local_1] == 1)))) || ((this._berilaChangedInteraction[_local_1] == 3))))
                    {
                        InteractiveObject(_local_1).mouseEnabled = false;
                    };
                    if ((((_local_1 is DisplayObjectContainer)) && ((this._berilaChangedInteraction[_local_1] > 1))))
                    {
                        DisplayObjectContainer(_local_1).mouseChildren = false;
                    };
                };
                this._berilaChangedInteraction = new Dictionary(true);
            };
...
class ShortcutItem 
{
    ...
    public function ShortcutItem(legend:String, key:uint, callback:Function, ctrl:Boolean, shift:Boolean, alt:Boolean)
    {
        this.legend = legend;
        this.key = key;
...
class InteractiveItemUi extends InteractiveItem 
{
    ...
    public function InteractiveItemUi()
    {
        super();
    ...
    override public function highlight(index:uint, elements:Vector.<InteractiveItem>):void
    {
        ...
        if ((target is UiRootContainer))
        {
            if (!(firstUi))
            {
                ...
                _highlightShape2.graphics.lineStyle(2, 0xFF, 0.7);
                _highlightShape2.graphics.beginFill(0xFF, 0);
        ...
        else
        {
            if (!(firstElem))
            {
                ...
                _highlightShape.graphics.beginFill(0xFF0000, 0);
    ...
    override public function tooltip():String
    {
        ...
        var _local_4:Vector.<BeriliaUiElementSound>;
        var soundParam:BeriliaUiElementSound;
        var str:String = "";
        var color:String = "#0";
        ...
        else
        {
            ...
            _local_4 = UiSoundManager.getInstance().getAllSoundUiElement((target as GraphicContainer));
            str = (str + (("<b>Sons : </b>" + ((_local_4.length) ? "" : "Aucun")) + ""));
            if (_local_4.length)
            {
                for each (soundParam in _local_4)
                {
...
class InteractiveItemCell extends InteractiveItem 
{
...

    {
        ...
        _selection.color = new Color(0xFF0000);
    }

    public function InteractiveItemCell()
    {
        super();
        shortcuts = Vector.<ShortcutItem>([new ShortcutItem("Cmd click test fonctionnel", Keyboard.Q, function ():void
        {
...
class InteractiveItemEntity extends InteractiveItem 
{
    ...
    public function InteractiveItemEntity()
    {
        super();
        shortcuts = Vector.<ShortcutItem>([new ShortcutItem("Cmd click test fonctionnel", Keyboard.Q, function ():void
        {
    ...
    override public function tooltip():String
    {
        ...
        var info:GameContextActorInformations;
        var npc:Npc;
        var monsterGroup:GameRolePlayGroupMonsterInformations;
        var monsters:Vector.<MonsterInGroupLightInformations>;
        var npcAction:NpcAction;
        var action:uint;
        var m:MonsterInGroupLightInformations;
        var monster:Monster;
...
class InteractiveItemElement extends InteractiveItem 
{
    ...
    public function InteractiveItemElement()
    {
        super();
        shortcuts = Vector.<ShortcutItem>([new ShortcutItem("Cmd click test fonctionnel", Keyboard.Q, function ():void
        {
...
class InteractiveItemMapBorder extends InteractiveItem 
{
    ...
    public function InteractiveItemMapBorder()
    {
        super();
        shortcuts = Vector.<ShortcutItem>([new ShortcutItem("Cmd click test fonctionnel", Keyboard.Q, function ():void
        {
