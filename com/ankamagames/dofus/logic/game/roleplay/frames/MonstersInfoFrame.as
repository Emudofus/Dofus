package com.ankamagames.dofus.logic.game.roleplay.frames
{
    ...
    public class MonstersInfoFrame implements Frame 
    {
        ...
        public function process(pMsg:Message):Boolean
        {
            ...
            var _local_4:EntityMouseOutMessage;
            var _local_5:GameContextRemoveElementMessage;
            var _local_6:EntityMovementStartMessage;
            var _local_7:EntityMovementCompleteMessage;
            var rootEntity:AnimatedCharacter;
            var groupIndex:int;
            ...
            switch (true)
            {
                case (pMsg is EntityMouseOverMessage):
                    _local_3 = (pMsg as EntityMouseOverMessage);
                    ac = (_local_3.entity as AnimatedCharacter);
                    if (ac)
                    {
                ...
                case (pMsg is EntityMouseOutMessage):
                    _local_4 = (pMsg as EntityMouseOutMessage);
                    ac = (_local_4.entity as AnimatedCharacter);
                    if (ac)
                    {
                ...
                case (pMsg is GameContextRemoveElementMessage):
                    _local_5 = (pMsg as GameContextRemoveElementMessage);
                    delete this._tooltipsCacheNames[_local_5.id];
                    TooltipManager.hide(("MonstersInfo_" + _local_5.id));
                    break;
                case (pMsg is EntityMovementStartMessage):
                    _local_6 = (pMsg as EntityMovementStartMessage);
                    ac = (EntitiesManager.getInstance().getEntity(_local_6.id) as AnimatedCharacter);
                ...
                case (pMsg is EntityMovementCompleteMessage):
                    _local_7 = (pMsg as EntityMovementCompleteMessage);
                    ac = (EntitiesManager.getInstance().getEntity(_local_7.id) as AnimatedCharacter);
                    if (ac == ac.getRootEntity())
                    {
                    ...
                    if (this._roleplayEntitiesFrame.monstersIds.indexOf(_local_7.id) != -1)
                    {
                        this.update(true);
                    };
                    break;
        ...
        private function showToolTip(pMonsterId:int, pCacheName:String=null):void
        {
            var offset:int;
            var ac:AnimatedCharacter;
            var data:GameRolePlayGroupMonsterInformations;
