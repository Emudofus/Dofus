package com.ankamagames.dofus.logic.game.common.frames
{
   ...
   public class PlayedCharacterUpdatesFrame extends Object implements Frame
   {
      ...
      public function process(msg:Message) : Boolean {
         ...
         switch(true)
         {
            ...
            case msg is CompassUpdateMessage:
               ...
               superAreaId=PlayedCharacterManager.getInstance().currentSubArea.area.superAreaId;
               KernelEventsManager.getInstance().processCallback(HookList.AddMapFlag,name,legend,superAreaId,cumsg.worldX,cumsg.worldY,color,false);
               return true;
            default:
               return false;
         }
      ...
      public function updateCharacterStatsList(cslmsg:CharacterStatsListMessage) : void {
         var oldSM:Vector.<CharacterSpellModification> = null;
         var newSM:Vector.<CharacterSpellModification> = null;
         var lengthSM:* = 0;
         var idSpellsToRefresh:Array = null;
         var iSM:* = 0;
         ...
         if(lastCharacteristics)
         {
            ...
            if((cslmsg.stats.alignmentInfos.dishonor<0)&&(!(lastCharacteristics.alignmentInfos.dishonor==cslmsg.stats.alignmentInfos.dishonor)))
            {
               KernelEventsManager.getInstance().processCallback(SocialHookList.DishonourChanged,cslmsg.stats.alignmentInfos.dishonor);
            }
            ...
            oldSM=PlayedCharacterManager.getInstance().characteristics.spellModifications;
            newSM=cslmsg.stats.spellModifications;
            lengthSM=Math.max(oldSM.length,newSM.length);
            idSpellsToRefresh=new Array();
            iSM=0;
            while(iSM<lengthSM)
            {
               if(oldSM.length<=iSM)
