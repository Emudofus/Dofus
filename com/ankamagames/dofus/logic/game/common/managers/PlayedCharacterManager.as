package com.ankamagames.dofus.logic.game.common.managers
{
   ...
   public class PlayedCharacterManager extends Object implements IDestroyable
   {
      ...
      public function get isIncarnation() : Boolean {
         ...
         var equipmentView:Object = InventoryManager.getInstance().inventory.getView("equipment");
         for each (item in equipmentView.content)
         {
            if(item)
            {
               for each (effect in item.effectsList)
               {
                  if(effect.actionId==669)
                  {
                     return true;
                  }
               }
