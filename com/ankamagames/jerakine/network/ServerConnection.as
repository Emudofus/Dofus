      ...
      protected function lowReceive(param1:IDataInput) : INetworkMessage
      {
         ...
         if(!this._splittedPacket)
         {
            if(param1.bytesAvailable < 2)
            {
               if(DEBUG_LOW_LEVEL_VERBOSE)
               {
                  _log.info("Not enought data to read the header, byte available : " + param1.bytesAvailable + " (needed : 2)");
               }
               return null;
            }
            ...
            if(param1.bytesAvailable >= (_loc3_ & NetworkMessage.BIT_MASK))
            {
               ...
               if(DEBUG_LOW_LEVEL_VERBOSE)
               {
                  _log.info("Not enought data to read msg content, byte available : " + param1.bytesAvailable + " (needed : " + _loc5_ + ")");
               }
            ...
            if(DEBUG_LOW_LEVEL_VERBOSE)
            {
               _log.info("Not enought data to read message ID, byte available : " + param1.bytesAvailable + " (needed : " + (_loc3_ & NetworkMessage.BIT_MASK) + ")");
            }
