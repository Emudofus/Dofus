package com.ankamagames.berilia.components
{
    ...
    public class Slot extends ButtonContainer implements ISlotDataHolder, FinalizableUIComponent, IDragAndDropHandler 
    {
        ...
        private const _quantityTextFormat:TextFormat = new TextFormat("Tahoma", 15, 0xFFFFFF);
        ...
        public function updateQuantity(num:int):void
        {
            ...
            this._quantitySprite.graphics.beginFill(0x333333, 0.6);
        ...
        private function updateTimer(t:int):void
        {
            ...
            else
            {
                ...
                this._timerStartTime = this._data.startTime;
                _local_3 = Math.round((((currentTime - this._timerStartTime) / this._timerMaxDuration) * 100));
                _local_3 = (100 - _local_3);
                this._tx_timerForeground.gotoAndStop = _local_3;
            };
        ...
        override public function process(msg:Message):Boolean
        {
            ...
            var holder:*;
            var t1:uint;
            var dropTargetHandler:IDragAndDropHandler;
            var dragData:SlotDragAndDropData;
            var currentHolder:ISlotDataHolder;
            var listener2:IInterfaceListener;
            var listener3:IInterfaceListener;
            if (this._isButton)
            {
                tmpState = 9999;
            ...
            switch (true)
            {
                ...
                case (msg is MouseReleaseOutsideMessage):
                    _local_6 = MouseReleaseOutsideMessage(msg).mouseEvent.target;
                    linkCursor = LinkedCursorSpriteManager.getInstance().getItem(DRAG_AND_DROP_CURSOR_NAME);
                    if (((((linkCursor) && (this._dragging))) && (!((_local_6 is ISlotDataHolder)))))
                    {
                        holder = SecureCenter.secure(SlotDragAndDropData(linkCursor.data).currentHolder);
                        switch (true)
                        {
                            case (_local_6 is IDragAndDropHandler):
                                if ((_local_6 as IDragAndDropHandler).dropValidator != null)
                                {
                                    dropTargetHandler = (_local_6 as IDragAndDropHandler);
                                ...
                                break;
                            case (_local_6 is MovieClip):
                            case (_local_6 is TextField):
                            case (_local_6 is Stage):
                                KernelEventsManager.getInstance().processCallback(BeriliaHookList.SlotDropedNorBeriliaNorWorld, holder);
                                break;
                            case (getQualifiedClassName(_local_6.parent).indexOf("com.ankamagames.berilia") >= 0):
                                KernelEventsManager.getInstance().processCallback(BeriliaHookList.SlotDropedOnBerilia, holder, _local_6);
                                break;
                            case Boolean(((_local_6.parent) && ((_local_6.parent.parent is MapViewer)))):
                            case Boolean(((_local_6.parent) && ((getQualifiedClassName(_local_6.parent).indexOf("Dofus") >= 0)))):
                                KernelEventsManager.getInstance().processCallback(BeriliaHookList.SlotDropedNorBeriliaNorWorld, holder, _local_6);
                                break;
                            default:
                                KernelEventsManager.getInstance().processCallback(BeriliaHookList.SlotDropedOnWorld, holder, _local_6);
                        };
                    ...
                    else
                    {
                        if ((_local_6 is Slot))
                        {
                            if ((_local_6 as Slot).allowDrag == false)
                            {
...
class DragSprite extends Sprite 
{
    ...
    public function DragSprite(bitmapData:BitmapData)
    {
        alpha = 0.8;
