package ui
{
   ...
   public class CartographyBase extends AbstractCartographyUi
   {
      ...
      public function addFlag(flagId:String, flagLegend:String, x:int, y:int, color:int = -1, playSound:Boolean = true, needMapUpdate:Boolean = true) : void {
         var uri:String = null;
         if(playSound)
         {
         ...
         var flag:Object = mapViewer.addIcon("layer_8",flagId,uri,x,y,this.__iconScale,flagLegend,true,color);
         if(flag)
         {
      ...
      protected function updatePrismAndSubareaStatus(prismSubAreaInformation:PrismSubAreaWrapper, pUpdateConquestModeData:Boolean = true) : void {
         ...
         var currentAreaItem:Object = null;
      ...
      public function onMapRollOver(target:Object, x:int, y:int) : void {
         ...
         var subAreaChanged:Boolean = false;
         if((mapIds) || (forcePrismSubArea))
         {
            ...
            if((subarea) && (subarea.worldmap) && (subarea.worldmap.id == _currentWorldId))
            {
               ...
               if((prismSubAreaInfo) && ((!(prismSubAreaInfo.mapId == -1)) || (prismSubAreaInfo.alliance)))
               {
                  ...
                  else if(this._allianceEmblemsLoaded)
                  {
      ...
      public function onTextureReady(target:Object) : void {
         ...
         if(this.__showAllianceEmblem)
         {
            ...
            else if(target == this.tx_emblemUp)
            {
      ...
      private function selectZoomButton() : void {
...
class MapPreset extends Object
{
   
   function MapPreset(mapPixelPosition:Object, zoomFactor:Number) {
      super();
