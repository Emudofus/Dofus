package ui
{
    ...
    public class CompanionTab 
    {
        ...
        public function main(oParam:Object=null):void
        {
            ...
            this._color1 = this.uiApi.me().getConstant("color1");
            this._color2 = this.uiApi.me().getConstant("color2");
            this._illusUri = this.uiApi.me().getConstant("illus");
            var iw:ItemWrapper = this.inventoryApi.getEquipementItemByPosition(CharacterInventoryPositionEnum.INVENTORY_POSITION_COMPANION);
            var companionItems:Array = new Array();
            var isEthereal:Boolean;
            if (iw)
            {
        ...
        {
            var carac:CompanionCharacteristic;
            var caracId:int;
        ...
        {
            var compSpell:CompanionSpell;
            var sw:SpellWrapper;
            var gradeByLevel:Array;
            var compSpellId:int;
        ...
        public function onRelease(target:Object):void
        {
            ...
            switch (target)
            {
                ...
                case this.btn_spells:
                    ...
                    break;
                case this.btn_equip:
                    _local_2 = this._myCompanions[this._selectedCompanion.id];
                    if (_local_2)
                    {
                        if (_local_2.item.position == CharacterInventoryPositionEnum.INVENTORY_POSITION_COMPANION)
                        {
                            this.sysApi.sendAction(new ObjectSetPosition(_local_2.item.objectUID, 63, 1));
                        }
                        else
                        {
                            this.sysApi.sendAction(new ObjectSetPosition(_local_2.item.objectUID, CharacterInventoryPositionEnum.INVENTORY_POSITION_COMPANION, 1));
                        };
                    };
                    break;
            };
        ...
        public function onObjectModified(item:Object):void
        {
            ...
            {
                if ((((item.position == 63)) && ((this._currentlyEquipedGID == item.objectGID))))
                {
                    if (this._myCompanions[this._selectedCompanion.id].item.objectGID == this._currentlyEquipedGID)
                    {
