        ...
        public function onRelease(target:Object):void
        {
            ...
            switch (target)
            {
                ...
                default:
                    ...
                    {
                        _local_2 = this._interactiveComponentsList[target.name];
                        if (_local_2.type == "Friend")
                        {
                            this.sysApi.sendAction(new RemoveFriend(_local_2.accountId));
                        }
                        else
                        {
                            if (_local_2.type == "Enemy")
                            {
                                this.sysApi.sendAction(new RemoveEnemy(_local_2.accountId));
                            }
                            else
                            {
                                if (_local_2.type == "Ignored")
                                {
                                    this.sysApi.sendAction(new RemoveIgnored(_local_2.accountId));
                                };
                            };
                        };
                    }
                    else
                    {
                        ...
                        {
                            _local_2 = this._interactiveComponentsList[target.name];
                            if (_local_2.state == PlayerStateEnum.GAME_TYPE_FIGHT)
                            {
                                this.sysApi.sendAction(new GameFightSpectatePlayerRequest(_local_2.playerId));
                            };
                        };
                    };
            };
