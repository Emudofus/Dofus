package ui
{
    ...
    import d2actions.ExchangeMultiCraftSetCrafterCanUseHisRessources;
    ...
    public class SmithMagicCoop extends SmithMagic 
    {
        ...
        private function _isExchangeValid():Boolean
        {
            ...
            for each (dpi in this.gd_bag.dataProvider)
            {
                ...
                {
                    if (((!((item.id == SIGNATURE_RUNE_ID))) || (!((slot_sign.data == null)))))
                    {
                        hasItem = true;
                    };
                };
                ...
                case this.gd_bag:
                    ...
                    switch (selectedMethod)
                    {
                        ...
                        case SelectMethodEnum.CTRL_DOUBLE_CLICK:
                            _local_4 = target.selectedItem.quantity;
                            break;
                    ...
                    if (_local_4 != -1)
                    {
                        if (this._isCrafter)
                        {
                            fillDefaultSlot(target.selectedItem, _local_4);
                            this._setItemOwner(target.selectedItem, false);
                        }
                        else
                        {
                            this._unfillBag(target.selectedItem, _local_4);
                            this._setItemOwner(target.selectedItem, false);
                        };
                    };
            };
        ...
        override protected function onExchangeObjectRemoved(itemUID:int, remote:Object):void
        {
            super.onExchangeObjectRemoved(itemUID, remote);
        }
