package ui
{
    ...
    public class SpectatorUi 
    {
        ...
        {
            var name:String;
            var comp:GameFightFighterCompanionLightInformations;
        ...
        public function updateFightLine(data:*, components:*, selected:Boolean):void
        {
            ...
            if (data)
            {
                ...
                else
                {
                    ...
                    {
                        if ((((((((data.type == FightTypeEnum.FIGHT_TYPE_AGRESSION)) || ((data.type == FightTypeEnum.FIGHT_TYPE_PvT)))) || ((data.type == FightTypeEnum.FIGHT_TYPE_PvPr)))) || ((data.type == FightTypeEnum.FIGHT_TYPE_Koh))))
                        {
        ...
        public function onRelease(target:Object):void
        {
            ...
            switch (target)
            {
                case this.btn_fightLeft:
                    _local_2 = this.roleplayApi.getFight(this._selectedFight.id);
                    if (!(_local_2))
                    {
                        return;
                    };
                    this.sysApi.sendAction(new JoinFightRequest(this._selectedFight.id, _local_2.teams[0].teamInfos.leaderId));
                    break;
                case this.btn_fightRight:
                    ...
                    if (_local_2.teams[1].teamType == TeamTypeEnum.TEAM_TYPE_TAXCOLLECTOR)
                    {
                        this.sysApi.dispatchHook(OpenSocial, 1, 2);
                    }
                    else
                    {
                        this.sysApi.sendAction(new JoinFightRequest(this._selectedFight.id, _local_2.teams[1].teamInfos.leaderId));
                    };
                    break;
