package ui
{
    ...
    public class ReadingBook extends DocumentBase 
    {
        ...
        private function _updateButtons():void
        {
            ...
            {
                if ((this._currentIndex + 1) >= this._nbPages)
                {
        ...
        public function onTextClick(target:Object, textEvent:String):void
        {
            ...
            {
                if (textEvent.indexOf(CLICKPAGE) != -1)
                {
                    uiApi.hideTooltip();
        ...
        {
            var image:ImageData;
            var bmpData:BitmapData = new BitmapData(lbl.width, lbl.height, true, 0xFF0000);
            ...
            else
            {
                for each (image in images)
                {
                    ...
                    if (image.before != "")
                    {
                        ...
                        this.addTextToBitmap(bmpData, lbl, m);
                        ypos = (ypos + lbl.textHeight);
                    };
                    this.addImageToBitmap(bmpData, image, ypos, page);
                    ypos = (ypos + image.height);
        ...
        private function addTextureOnPage(page:GraphicContainer, target:Object):void
        {
        ...
        private function formateLinks(lbl:TextArea, bmpData:BitmapData):void
        {
            ...
            for each (link in links)
            {
                ...
                link.setGraphicData(ctr, this.mainCtr, rect, new Point(lbl.x, lbl.y));
        ...
        override public function copyAllDataToClipBoard():void
        {
            ...
            while (i < len)
            {
                result = (result + (this._pages[i] + (((i < (len - 1))) ? "<pagefeed/>" : "")));
                i++;
            };
