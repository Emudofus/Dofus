package ui
{
    ...
    public class ItemBidHouseSell extends BasicItemCard 
    {
        ...
        public function onSelectItemFromInventory(selectedItem:Object):void
        {
            ...
            var allowedType:Boolean;
            var nonExchangeable:Boolean;
            var nType:int = this._sellerDescriptor.types.length;
            var t:int;
            while (t < nType)
            {
                if (this._sellerDescriptor.types[t] == _currentObject.typeId)
                {
                    allowedType = true;
            ...
            else
            {
                ...
                {
                    if (_currentObject.level > this._sellerDescriptor.maxItemLevel)
                    {
                    ...
                    else
                    {
                        ...
                        if (selectedItem.name == selectedItem.realName)
                        {
                            if (TradeCenter.SALES_QUANTITIES[selectedItem.objectGID])
                            {
        ...
        private function updateTax():void
        {
            ...
            if (((this._mode) || (!((currentPrice == this._oldPrice)))))
            {
                ...
                else
                {
                    ...
                    {
                        if (this._oldPrice < currentPrice)
                        {
        ...
        private function onExchangeBidPriceForSeller(genericId:uint, averagePrice:int, minimalPrices:Object, allIdentical:Boolean):void
        {
            ...
            var minimalPricesExist:Boolean;
        ...
        override public function onRelease(target:Object):void
        {
            ...
            {
                if ((((target == btn_remove)) && (!(this._mode))))
                {
                ...
                {
                    if ((((target == btn_modify)) && (!(this._mode))))
                    {
                        this._price = utilApi.stringToKamas(input_price.text, "");
