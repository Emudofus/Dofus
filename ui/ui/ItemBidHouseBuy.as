    ...
    public class ItemBidHouseBuy 
    {
        ...
        private function refreshbidObjectListStep(keepScroll:Boolean, lcFilter:String, gridProvider:Array, cbDictionary:Dictionary, nObjects:uint, offset:uint=0):void
        {
            ...
            while (i < nObjects)
            {
                if ((getTimer() - ts) > 20)
                {
                    ...
                    uiPopup = this.uiApi.getUi(this._progressPopupName);
                    if (((uiPopup) && (uiPopup.getElement("lbl_content"))))
                    {
                        uiPopup.getElement("lbl_content").text = this.uiApi.getText("ui.popup.processingObject", Math.floor(((i / nObjects) * 100)).toString());
                    };
                    this._searchSettimoutId = setTimeout(this.refreshbidObjectListStep, 1, keepScroll, lcFilter, gridProvider, cbDictionary, nObjects, i);
                    return;
                };
                item = this._listObjects[i];
                if (!(this.filtered(lcFilter, item.itemWrapper)))
                {
                    groupSize = 8;
                    ...
                    gridProvider.push({
                        ...
                        "name":item.itemWrapper.shortName,
                        "prices":item.prices,
                        "currentCase":-1,
                        "p1":item.prices[0],
                        "p2":item.prices[1],
                ...
                if (this._detailView)
                {
                    this.addToCombobox(cbDictionary, item.itemWrapper.shortName);
                };
                i++;
            };
            this.refreshbidObjectListEnd(keepScroll, gridProvider, cbDictionary);
        }
        ...
        private function filtered(filter:String, item:ItemWrapper):Boolean
        {
            ...
            for each (effect in item.effects)
            {
                switch (effect.effectId)
                {
                    case 623:
                    case 628:
                        _local_4 = this.dataApi.getMonsterFromId(int(effect.parameter2));
                        if (_local_4.name.toLowerCase().indexOf(filter) != -1)
                        {
                            return (false);
                        };
                        break;
        ...
        public function onRelease(target:Object):void
        {
            ...
            else
            {
                ...
                {
                    if ((((target == this.btn_tabQty1)) || ((target == this.btn_price))))
                    {
