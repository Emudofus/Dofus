package ui
{
   ...
   public class PrismDefense extends Object
   {
      ...
      public function onSelectItem(target:Object, selectMethod:uint, isNewSelection:Boolean) : void {
         ...
         {
            case this.gd_defense:
               item = this.gd_defense.selectedItem;
               if(item)
               {
                  if(item.playerCharactersInformations.id == this.playerApi.id())
                  {
                     this.sysApi.sendAction(new PrismFightJoinLeaveRequest(0,false));
                  }
                  else
                  {
                     this.sysApi.sendAction(new PrismFightSwapRequest(0,item.playerCharactersInformations.id));
                  }
               }
               else
               {
