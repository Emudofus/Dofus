package ui
{
   ...
   public class CartographyPocket extends CartographyBase
   {
      ...
      public function CartographyPocket() {
         super();
      }
      
      private static const HINT_RES_NAMES:Array;
      
      private static const HINT_LAYOUT_INDEX:Array;
      ...
      override public function main(params:Object = null) : void {
         ...
         uiApi.addComponentHook(this.subCtr,ComponentHookList.ON_ROLL_OUT);
         ...
         uiApi.addComponentHook(this.tx_bitmap,ComponentHookList.ON_ROLL_OVER);
      ...
      override public function addFlag(flagId:String, flagLegend:String, x:int, y:int, color:int = -1, playSound:Boolean = true, needMapUpdate:Boolean = true) : void {
         var uri:String = null;
         if(playSound)
         {
         ...
         var flag:Object = mapViewer.addIcon("layer_8",flagId,uri,x,y,__iconScale,flagLegend,true,color);
         if(flag)
         {
            if(needMapUpdate)
            {
      ...
      override public function onEnterFrame() : void {
         ...
         if(ctrSubAreaInfo.visible)
         {
            ...
            if(uiApi.getMouseX() + ctrSubAreaInfoBg.width > mapViewer.width - mapViewer.roundCornerRadius)
            {
               ctrSubAreaInfo.x = ctrSubAreaInfo.x - (ctrSubAreaInfoBg.width + mapViewer.roundCornerRadius);
            }
            if(uiApi.getMouseY() + ctrSubAreaInfoBg.height > mapViewer.y + mapViewer.height)
            {
         ...
         if((!this._contextMenuIsActive) && (!mapViewer.isDragging))
         {
            ...
            else if((this.tx_bitmap.visible) && (!this._mouseIsOut))
            {
               this.hideBitmap();
            }
      ...
      override public function onRelease(target:Object) : void {
         ...
         var opacityMenu:Array = null;
         var i:* = 0;
         var layerName:String = null;
         var layerId:* = 0;
         var hintIsSelected:* = false;
         ...
         switch(target)
         {
            ...
            case this.btn_hints:
               ...
               iconUri = uiApi.me().getConstant("assets");
               i = 0;
               while(i < 9)
               {
                  ...
                  contextMenu.push(modContextMenu.createContextMenuPictureItemObject(iconUri + HINT_RES_NAMES[i] + "?5",this.onFilterHintClick,[layerName,layerId],false,null,hintIsSelected,false,HINT_TOOLTIP_TEXT[i],false,200));
                  i++;
               }
               modContextMenu.createContextMenu(contextMenu,new Point(this.popCtr.x + this.btn_hints.x + this.btn_hints.width + 4,this.popCtr.y + this.btn_hints.y));
               break;
            case this.btn_options:
               contextMenu = new Array();
               opacityMenu = [modContextMenu.createContextMenuItemObject("0%",this.onOpacityChanged,[0],false,null,this._inactiveAlpha == 0),modContextMenu.createContextMenuItemObject("25%",this.onOpacityChanged,[0.25],false,null,this._inactiveAlpha == 0.25),modContextMenu.createContextMenuItemObject("50%",this.onOpacityChanged,[0.5],false,null,this._inactiveAlpha == 0.5),modContextMenu.createContextMenuItemObject("75%",this.onOpacityChanged,[0.75],false,null,this._inactiveAlpha == 0.75),modContextMenu.createContextMenuItemObject("100%",this.onOpacityChanged,[1],false,null,this._inactiveAlpha == 1)];
               contextMenu.push(modContextMenu.createContextMenuItemObject(uiApi.getText("ui.cartography.opacitymenu"),null,null,false,opacityMenu));
               modContextMenu.createContextMenu(contextMenu,new Point(this.popCtr.x + this.btn_options.x + this.btn_options.width + 4,this.popCtr.y + this.btn_options.y));
               break;
         }
      }
      ...
      private function onFilterHintClick(layerName:String, layerId:int, forceSelection:Boolean = false) : void {
         var contextMenuItems:Array = null;
         var item:* = undefined;
      ...
      private function onMapChanged(map:WorldPoint) : void {
         ...
         var worldId:int = subArea.hasCustomWorldMap?subArea.area.superArea.worldmap.id:subArea.worldmap.id;
      ...
      private function updateComponentPosition() : void {
         ...
         mapViewer.width = this.tx_resizeShape.width - 2;
         mapViewer.height = this.tx_resizeShape.height - 35;
         ...
         this.tx_bg.height = this.tx_resizeShape.height - 34;
      ...
      private function forceUiWithinScreenBounds() : void {
         ...
         else if(this.popCtr.x + this.tx_bitmap.width * 0.4 > stageWidth)
         {
         ...
         else if(this.popCtr.y > stageHeight - this.tx_bitmap.height)
         {
