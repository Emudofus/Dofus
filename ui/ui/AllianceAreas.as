package ui
{
   ...
   public class AllianceAreas extends Object
   {
      ...
      private function updateSubAreas() : void {
         ...
         this.gd_subAreas.dataProvider = this._prismsList;
         var i:int = 0;
         while(i < this._prismsList.length)
         {
            if(this._forceShowPrismId == this._prismsList[i].subAreaId)
            {
         ...
         switch(target)
         {
            ...
            default:
               if(target.name.indexOf("lbl_coord") != -1)
               {
                  ...
                  px = data.worldX;
                  py = data.worldY;
                  this.sysApi.dispatchHook(AddMapFlag,"flag_srv" + CompassTypeEnum.COMPASS_TYPE_PVP_SEEK + "_pos_" + px + "_" + py,px + "," + py + " (" + this.uiApi.getText("ui.zaap.prism") + ")",this.mapApi.getCurrentWorldMap().id,px,py,16711680,true);
               }
