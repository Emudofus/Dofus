package ui
{
    ...
    public class StockBidHouse 
    {
        ...
        {
            var objInSale:ObjectItemToSellInBid;
            var nItems:int;
            var i:int;
        ...
        public function changeBidTooltip(sellerBuyerDescriptor:Object):void
        {
            ...
            while (i < nTypes)
            {
                type = this.dataApi.getItemType(sellerBuyerDescriptor.types[i]);
                if (type)
                {
                    lsTypes[i] = type.name;
                };
        ...
        private function comboBoxBuyMode():void
        {
            ...
            while (i < nTypes)
            {
                ...
                type = this.dataApi.getItemType(id);
                cbProvider[i] = {
                    "label":((type) ? type.name : "?"),
                    "type":id
                };
                this._groupCategoriesId = (this._groupCategoriesId + id.toString());
                i++;
            };
            cbProvider = cbProvider.sortOn("label");
            this.cbFilter.dataProvider = cbProvider;
            var savedIndex:* = this.sysApi.getSetData(("buyModeLastSelectedType_" + this._groupCategoriesId), 0);
            this.cbFilter.selectedIndex = savedIndex;
        }
            ...
            switch (target)
            {
                ...
                case this.cbFilter:
                    if (TradeCenter.BID_HOUSE_BUY_MODE)
                    {
                        ...
                        this.sysApi.setData(("buyModeLastSelectedType_" + this._groupCategoriesId), target.selectedIndex);
