   ...
   public class WorldRpCharacterTooltipUi extends AbstractWorldCharacterTooltipUi
   {
      ...
      public function main(oParam:Object = null) : void {
         ...
         if(hasInfos)
         {
            ...
            if(data.infos.humanoidInfo is HumanInformations)
            {
               for each(option in (data.infos.humanoidInfo as HumanInformations).options)
               {
                  ...
                  else if(option is HumanOptionAlliance)
                  {
         ...
         if(this.lbl_title.text != "")
         {
            ...
            if((hasGuildInformation) && (hasAllianceInformation))
            {
               w = w + (this.tx_AllianceEmblemBack.width + 8);
            }
            if(w < this.lbl_title.width + 8)
            {
               ...
               if(hasGuildInformation)
               {
                  ...
                  {
                     center = (this.lbl_title.x + this.lbl_title.width + 8) / 2;
                     blockWidth = this.tx_emblemBack.width + 8;
                     if(this.lbl_guildName.width > this.lbl_playerName.width)
                     {
                        blockWidth = blockWidth + this.lbl_guildName.width;
                     }
                     else
                     {
                        blockWidth = blockWidth + this.lbl_playerName.width;
                     }
                     this.tx_emblemBack.x = center - blockWidth / 2;
                     this.tx_emblemUp.x = this.tx_emblemBack.x + 8;
                     this.lbl_guildName.x = this.lbl_playerName.x = this.tx_emblemBack.x + this.tx_emblemBack.width + 2;
                  }
                  else
                  {
         ...
         else if(hasGuildInformation)
         {
            ...
            if(hasAllianceInformation)
            {
               if(this.lbl_guildName.width > this.lbl_playerName.width)
               {
                  this.tx_back.width = this.lbl_guildName.x + this.lbl_guildName.width + this.tx_AllianceEmblemBack.width + 16;
               }
               else
               {
                  this.tx_back.width = this.lbl_playerName.x + this.lbl_playerName.width + this.tx_AllianceEmblemBack.width + 16;
               }
            }
