package flashx.textLayout.container
{
   ...
   public class ContainerController extends Object implements IInteractionEventHandler, ITextLayoutFormat, ISandboxSupport
   {
      ...
      public function scrollToRange(activePosition:int, anchorPosition:int) : void {
         ...
         var horizontalScrollOK:* = false;
         var verticalScrollOK:* = false;
         ...
         var activePosition:int = Math.max(controllerStart,Math.min(activePosition,lastPosition));
         var anchorPosition:int = Math.max(controllerStart,Math.min(anchorPosition,lastPosition));
      ...
      tlf_internal function attachTransparentBackgroundForHit(justClear:Boolean) : void {
         ...
         var bgwidth:* = NaN;
         var bgheight:* = NaN;
         var adjustHorizontalScroll:* = false;
         var bgx:* = NaN;
         var bgy:* = NaN;
      ...
      private function autoScrollIfNecessaryInternal(extreme:Point) : int {
         ...
         {
            if(extreme.y-this.containerScrollRectTop<0)
            {
               this.verticalScrollPosition=this.verticalScrollPosition-this.textFlow.configuration.scrollDragPixels;
               scrollDirection=-1;
            }
         ...
         {
            if(extreme.x-this.containerScrollRectLeft<0)
            {
               this.horizontalScrollPosition=this.horizontalScrollPosition-this.textFlow.configuration.scrollDragPixels;
               scrollDirection=1;
            }
      ...
      public function getScrollDelta(numLines:int) : Number {
         var newLineIndex:* = 0;
         var lineIndex:* = 0;
         var newScrollPosition:* = NaN;
         var lastTextLine:TextLine = null;
         var previousDamageStart:* = 0;
         ...
         if(numLines>0)
         {
            ...
            {
               if(this.containerScrollRectLeft-lastTextLine.x-lastTextLine.descent>2)
               {
                  lineIndex--;
               }
            }
         ...
         if(numLines<0)
         {
            ...
            {
               if(firstVisibleLine.x+firstVisibleLine.ascent>this.containerScrollRectRight+2)
               {
                  lineIndex++;
               }
            }
      ...
      private function hitOnMyFlowExceptLastContainer(event:MouseEvent) : Boolean {
         ...
         else
         {
            ...
            {
               idx=0;
               while(idx<this.textFlow.flowComposer.numControllers-1)
               {
      ...
      tlf_internal function drawPointSelection(selFormat:SelectionFormat, x:Number, y:Number, w:Number, h:Number) : void {
         ...
         if(this._hasScrollRect)
         {
            ...
            {
               if(y>=this.containerScrollRectBottom)
               {
                  y=y-h;
               }
            }
      ...
      tlf_internal function addSelectionShapes(selFormat:SelectionFormat, selectionAbsoluteStart:int, selectionAbsoluteEnd:int) : void {
         ...
         var absoluteControllerStart:* = 0;
         var absoluteControllerEnd:* = 0;
         var begLine:* = 0;
         var endLine:* = 0;
         ...
         if(selectionAbsoluteStart!=selectionAbsoluteEnd)
         {
            ...
            {
               if(selectionAbsoluteStart>=absoluteControllerEnd)
               {
                  return;
               }
            }
            ...
            {
               if(selectionAbsoluteEnd<absoluteControllerStart)
               {
                  return;
               }
            }
            ...
            while(idx<=endLine)
            {
               ...
               line.hiliteBlockSelection(selObj,selFormat,this._container,selectionAbsoluteStart>line.absoluteStart?line.absoluteStart:selectionAbsoluteStart,selectionAbsoluteEnd<line.absoluteStart+line.textLength?line.absoluteStart+line.textLength:selectionAbsoluteEnd,prevLine,nextLine);
               temp=line;
               line=nextLine;
               prevLine=temp;
               idx++;
      ...
      tlf_internal function updateCompositionShapes() : void {
         ...
         if((this._updateStart<this.absoluteStart)&&(newShapeChildren.length<0))
         {
            ...
            if(shapeChildrenStartIdx>=0)
            {
               ...
               childIdx=childIdx+shapeChildrenStartIdx;
            }
            else
            {
         ...
         this._mouseEventManager.updateHitTests((this.effectiveBlockProgression==BlockProgression.RL)&&(this._hasScrollRect)?this._contentWidth:0,scratchRectangle,tf,firstVisibleLine?firstVisibleLine.absoluteStart:this._absoluteStart,lastVisibleLine?lastVisibleLine.absoluteStart+lastVisibleLine.textLength-1:this._absoluteStart,needsCtrlKey);
      ...
      tlf_internal function updateGraphics(updateStart:int) : void {
         ...
         var shouldDisplayGraphic:* = false;
         var index:* = 0;
         ...
         var lastVisiblePosition:int = lastLine?lastLine.absoluteStart+lastLine.textLength:this.absoluteStart+this.textLength;
         var followingLine:TextFlowLine = this.flowComposer.findLineAtPosition(lastVisiblePosition);
         var lastPossibleFloatPosition:int = followingLine?followingLine.absoluteStart+followingLine.textLength:this.absoluteStart+this.textLength;
         ...
         loop1:
         for(;floatIndex<numFloats;parent=i<this._shapeChildren.length?this._shapeChildren[i]:null)
         {
            ...
            if(floatInfo.floatType==Float.NONE)
            {
               ...
               if(tl)
               {
                  ...
                  if((!tfl)||(floatInfo.absolutePosition>tfl.absoluteStart)||(floatInfo.absolutePosition>=tfl.absoluteStart+tfl.textLength)||(tl.parent==null)||(tl.validity==TextLineValidity.INVALID))
                  {
                     ...
                     i=0;
                     while(i<this._shapeChildren.length)
                     {
      ...
      protected function getFirstTextLineChildIndex() : int {
         var firstTextLine:* = 0;
         firstTextLine=0;
         while(firstTextLine<this._container.numChildren)
         {
      ...
      protected function updateVisibleRectangle() : void {
         var contentRight:* = NaN;
         var contentBottom:* = NaN;
         var width:* = NaN;
         var compositionRight:* = NaN;
         var height:* = NaN;
         var compositionBottom:* = NaN;
         var xOrigin:* = NaN;
         var xpos:* = 0;
         var ypos:* = 0;
      ...
      tlf_internal function isLineVisible(wmode:String, scrollXTW:int, scrollYTW:int, scrollWidthTW:int, scrollHeightTW:int, textFlowLine:TextFlowLine, textLine:TextLine) : TextLine {
         ...
         if((wmode==BlockProgression.TB?this._measureHeight:this._measureWidth)||(textFlowLine.isLineVisible(wmode,scrollXTW,scrollYTW,scrollWidthTW,scrollHeightTW)))
         {
      ...
      tlf_internal function getPlacedTextLineBounds(textLine:TextLine) : Rectangle {
         ...
         curBounds.x=curBounds.x+textLine.x;
         curBounds.y=curBounds.y+textLine.y;
         return curBounds;
      }
